...
const QRLoginView = () => {
  ...
  return (
    <Container sx={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
      <Button onClick={handleQrRefresh}
        disabled={!refresh} variant="dashed" sx={{ borderRadius: 5, padding: 2, marginTop: 5 }}>
        <QRCode
          size={128}
          style={{ height: "auto", maxWidth: "50", opacity: refresh ? 0.1 : 1 }}
          value={"fgdfg"}
          viewBox={`0 0 256 256`}
        />

        <Box sx={{ position: "absolute", opacity: refresh ? 1 : 0 }}>
          <UiIcon iconName="refresh" classes={{
            width: 90,
            height: 90,
            fontSize: 90,
          }} />
